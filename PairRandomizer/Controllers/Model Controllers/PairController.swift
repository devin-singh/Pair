...
class PairController {
    ...
    func sortIntoPairs() {
        ...
        for _ in allData {
            ...
            // Now we have two people in our pair object and they are removed from the main array and pair obj inside pair array

            // Problem. We are removing from the array inside the for loop twice. We only want to do this if there are 2 or more elements remaining. If there are less than 2, return. If we only have one element, we don't want to randomize.
            if allData.count < 2 {
                return
            }
    ...
    func createFileForPersistence() -> URL {
        ...
        // Create the new fileURL on the users Phone
         let fileUrl = urls[0].appendingPathComponent("Pairsz.json")
         return fileUrl
     }
